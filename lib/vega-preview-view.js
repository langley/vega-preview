...
export default class VegaPreviewView {
  ...
  constructor(serializedState) {
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      console.log(`path to file: ${editor.getPath()}`)
      let vegaSpecJson;
      try {
...
function adjustVegaSpec(spec) {
  let vegaSpec = adjustVegaDataElement(spec);
  try {
...
function adjustVegaDataElement(spec) {
  let dataElement = spec['data'];
  if (typeof dataElement != 'undefined') {
    console.log(`typeof dataElement = ${typeof dataElement}`);
    let url = dataElement['url'];
    if (typeof url != 'undefined') {
      console.log(`vega data.url = ${url}`);
      // url.replace('\/\/\.','//fu')
      let adjustedUrl = url.replace('\/\/\.','//blah');
      console.log(`Adjusted vega data.url = ${adjustedUrl}`);
    } else {
